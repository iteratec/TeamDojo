...
export default class TeamSkill extends mixins(JhiDataUtils) {
  ...
  public retrieveAllTeamSkills(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.teamSkills.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: ITeamSkill): void {
  ...
  public removeTeamSkill(): void {
    this.teamSkillService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('teamDojoApp.teamSkill.deleted', { param: this.removeId });
