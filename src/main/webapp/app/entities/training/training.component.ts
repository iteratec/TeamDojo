...
export default class Training extends mixins(JhiDataUtils) {
  ...
  public retrieveAllTrainings(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.trainings.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: ITraining): void {
  ...
  public removeTraining(): void {
    this.trainingService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('teamDojoApp.training.deleted', { param: this.removeId });
